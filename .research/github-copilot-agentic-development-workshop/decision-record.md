...
The application is `samples/developer-workbench-mcp`: a local, secret-free MCP server that validates approved HTTPS learning links and writes a Netscape HTML bookmark export to a fixed repository path. It demonstrates the complete engineering loop: context, clarification, an assumption challenge, plan review, implementation, focused validation, explicit MCP write approval, and read-only quality review.
