...
function extractListings(
  ...
  for (const [index, match] of matches.entries()) {
    ...
    const serkoLocation = source.id === "serko" ? extractSerkoLocation(labelText) : null;
    const title = serkoLocation
      ? labelText.replace(serkoLocation, "").replace(/\b(?:full[- ]time|part[- ]time|contract)\b/gi, "").replace(/\s+/g, " ").trim()
      : labelText;
    if (!pattern.test(sourceUrl) || title.length < 3 || title.length > 160) continue;

    const nextMatch = matches[index + 1];
    const nextMatchIndex = nextMatch?.index ?? html.length;
    const location = source.id === "xero"
      ? extractXeroLocation(html.slice((match.index ?? 0) + match[0].length, nextMatchIndex))
      : serkoLocation;
    if ((source.id === "xero" || source.id === "serko") && !location?.endsWith("New Zealand")) continue;

    listings.set(sourceUrl, { title, location, summary: null, sourceUrl });
  }
