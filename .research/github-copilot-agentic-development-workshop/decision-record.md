...
**Decision: Rebuild this repository around a deterministic local TypeScript MCP server that exports categorized developer-learning bookmarks. Keep VS Code Insiders and Copilot CLI as the documented tracks, use Skills for Copilot Student and paid plans, and provide equivalent prompt fallbacks for Copilot Free.**

Students use a GitHub Copilot coding agent in VS Code Insiders or Copilot CLI to create `samples/developer-workbench-mcp` from an absent target directory. The resulting local, secret-free MCP server validates approved HTTPS learning links and writes a Netscape HTML bookmark export to a fixed repository path. No implementation or scaffold ships in the starter repository; the exercise demonstrates context, clarification, an assumption challenge, plan review, creation, focused validation, explicit MCP write approval, and read-only quality review.

Copilot Student, Pro, Pro+, and Max are fully supported. Business and Enterprise support depends on organization policy enabling MCP and Copilot CLI. Copilot Free can follow the core workflow but uses prompt fallbacks where interactive Skill invocation is unavailable; the workshop does not promise a numeric AI-credit allowance. GitHub Copilot App is recommended only as optional self-directed exploration for students already confident with VS Code Insiders and Copilot CLI. The workshop provides no Copilot App setup or usage instructions.
