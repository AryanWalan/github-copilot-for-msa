...
3. Confirm that GitHub Copilot is active in your chosen client.
...
Clone this repository, then run the local MCP project validation:
...
All three validation commands must pass before the workshop begins. Keep the command output available if one fails.

## 5. Trust and inspect MCP servers

1. Open the repository as a trusted workspace.
2. Review [.vscode/mcp.json](.vscode/mcp.json). It configures Microsoft Learn, Context7, and the local Developer Workbench MCP server.
3. Approve only the servers you understand.
4. Confirm that the local server exposes `add_learning_links` and `list_learning_links`.
